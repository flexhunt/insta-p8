...
export async function GET(request: NextRequest) {
    ...
    if (customToken) {
        ...
        // Sanitize: Remove "Bearer " prefix and surrounding quotes
        accessToken = customToken.trim().replace(/^Bearer\s+/i, "").replace(/^"|"$/g, '')
    }
    ...
    // Sanitize
    const spyToken = spyTokenRaw ? spyTokenRaw.trim().replace(/^Bearer\s+/i, "").replace(/^"|"$/g, '') : null
